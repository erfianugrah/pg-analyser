...
function trendSourceLabel(a: Analysis): string {
  switch (a.meta.trendSource) {
    ...
    case "store":
      return "from the metrics history store (pg-analyser snapshot)";
    case "import":
  ...
  return findings
    .map((f, i) => {
      const links = [
        ...
        ...(f.refs ?? []).map((r) => `<a href="${esc(r.url)}">${esc(r.label)} &#8599;</a>`),
...
<meta name=viewport content="width=device-width,initial-scale=1">${faviconTag(brand)}<title>pg-analyser - org report</title>
<style>
  ${themeVars(brand)}
...
export function renderOrgIndex(
  ...
  const body = rows
    ...
    .join("");
  return `<!doctype html><html lang=en><head><meta charset=utf-8>
<meta name=viewport content="width=device-width,initial-scale=1">${faviconTag(brand)}<title>pg-analyser - all organizations</title>
<style>
  ${themeVars(brand)}
  body{font:14px/1.45 -apple-system,Segoe UI,Roboto,sans-serif;color:var(--fg);background:var(--bg);margin:0 auto;padding:24px;max-width:1000px}
  h1{font-size:20px;margin:0 0 4px}.meta{color:var(--mut);font-size:12px;margin-bottom:16px}
...
${faviconTag(brand)}
<title>pg-analyser - ${esc(m.name)}</title>
<style>
  ${themeVars(brand)}
  ${BRAND_CSS}
...
<meta name=viewport content="width=device-width,initial-scale=1">
${faviconTag(brand)}
<title>pg-analyser narrative - ${esc(m.name)}</title>
<style>
  ${themeVars(brand)}
...
<p class=foot>Plain-language summary. A detailed technical report (report.html) accompanies this for the engineering team. Generated by pg-analyser ${esc(m.sbperfVersion)}.</p>
</body></html>`;
}

function poolerSection(a: Analysis): string {
  const rows = a.pooler ?? [];
  if (!rows.length) return `<p class=empty>no pooler configured</p>`;
  const body = rows
    .map(
      (p) =>
