...
export async function narrate(a: Analysis, client: LlmClient): Promise<string> {
  ...
  const header = `<!-- generated by pg-analyser narrate (${client.model}) from analysis.json; the deterministic report.html is ground truth -->\n\n`;
  return `${header}${text}\n`;
}
 ...
 * PG_ANALYSER_LLM_BASE_URL + PG_ANALYSER_LLM_MODEL required; PG_ANALYSER_LLM_API_KEY optional
...
export function clientFromEnv(
  ...
  const baseUrl = env.PG_ANALYSER_LLM_BASE_URL;
  const model = env.PG_ANALYSER_LLM_MODEL;
  if (!baseUrl || !model) {
    return {
      error:
        "narrate needs an LLM: set PG_ANALYSER_LLM_BASE_URL (e.g. http://localhost:11434/v1 or " +
        "https://api.openai.com/v1) and PG_ANALYSER_LLM_MODEL (PG_ANALYSER_LLM_API_KEY if the endpoint " +
        "requires a key).",
    };
  }
  return { client: new OpenAiCompatClient(baseUrl, model, env.PG_ANALYSER_LLM_API_KEY) };
}
