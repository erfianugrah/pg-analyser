...
describe("shipped example stays schema-valid", () => {
  test("pg-analyser.profile.example.json parses against the Profile schema", async () => {
    const p = parseProfile(await Bun.file("pg-analyser.profile.example.json").text());
