...
describe("renderAlertsYaml", () => {
  ...
  test("splits the rules that need an extra scrape source into -optional", () => {
    const y = yaml();
    expect(y).toContain("  - name: sbperf\n");
    expect(y).toContain("  - name: sbperf-optional\n");
    const optionalBlock = y.slice(y.indexOf("- name: sbperf-optional"));
  ...
  test("the header marks the file generated", () => {
    expect(yaml().split("\n")[0]).toContain("Generated by sbperf");
  });
});
