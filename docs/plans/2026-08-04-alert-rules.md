...
`/tmp/sbperf-check` (tar-pipe excluding `node_modules`, `.git`, and every gitignored
...
2. The pack is useful against an existing Prometheus that sbperf did not generate. That is
...
describe("renderAlertsYaml", () => {
  ...
  test("round-trips through a YAML parser with the label matchers intact", () => {
    const parsed = Bun.YAML.parse(renderAlertsYaml(rules, "sbperf-abc")) as ParsedPack;
    expect(parsed.groups.map((g) => g.name)).toEqual(["sbperf-abc", "sbperf-abc-optional"]);
...
```
sbperf-abcdefghijklmnopqrst 8
sbperf-abcdefghijklmnopqrst-optional 5
```
