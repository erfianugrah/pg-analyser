...
| `top_time_query` / `top_query_db_time` | `pg_stat_statements` top by total_exec_time; low attribution by default, med only with real per-call cost (mean >= 50 ms) OR a >= 3x share AND >= 10 min of total exec time in the window - a 44% share of an idle database (25 s in a week, measured) is not a MED. pg_net's worker statements (`net.http_request_queue` pop, `net._http_response` reaper) are platform noise and excluded | >= 10% of DB time | low/med | HAVE |
...
| `mem_pressure_paging` | `node_vmstat_pgmajfault` / `pswpin` rate (needs >=2 snapshots / a Prometheus), graded by the FRACTION of samples at/above the rate thresholds - not the window mean, which a few 500/s spikes lift past 20/s while the median is 0.4 (measured). Title quotes fraction, median and peak per signal | >= 25% of the window above 20 faults/s or 2 swap-in pages/s = sustained (med); >= 5% = episodic (low) | med/low | HAVE (trend) |
...
| `lock_wave` | retrospective server-log parse (superuser + probe-gated): a burst of "still waiting for ...Lock" + timeout-cancellation lines = a lock-queue cascade (DDL queued behind readers, then readers queued behind the DDL). Worst 10-min window drives severity; a deadlock line alone is >=MED. Requires LOCK EVIDENCE in the window (a wait, a lock-timeout cancel or a deadlock) - statement-timeout cancels count as cascade victims only then; a cancel-only burst is `statement_timeout_burst`. Evidence names the span of matched events and the bytes scanned (the newest tails), since a quiet tail is not an unscanned one. PRIVACY: only parsed counts + literal-free reconstructed sample phrases are stored, never raw log text | HIGH: lock evidence AND (cancels>=50, or maxWait>=60s & waits>=10); MED: lock evidence AND (waits>=10 / cancels>=10 / any deadlock) | high/med | HAVE |
| `statement_timeout_burst` | same log parse: >= 10 `canceling statement due to statement timeout` lines in a 10-min window with NO lock line at all - statements running past their fuse (Supabase roles: anon 3s, authenticated 8s), a batch job, a cold cache, an I/O stall. Measured: 74 cancels in two minutes, zero waits, previously titled a HIGH lock cascade. Evidence names the role-level statement_timeout values in force | MED >= 50 cancels; LOW >= 10 | med/low | HAVE |
